...
def get_all_currencies_rates_str():
    ...
    info=twder.now_all()
    for currency_code,currency_rates in info.items():
        all_currencies_rates_str+=f'[{currency_code}] 現金買入：{currency_rates[1]} 現金賣出:{currency_rates[2]} 即期買入：{currency_rates[3]} 即期賣出：{currency_rates[4]} ({currency_rates[0]})\n'
...
def handle_message(event):
    ...
    if user_input == '@查詢所有匯率':
        ...
        reply_message=TextSendMessage(text=all_currencies_rates_str)
